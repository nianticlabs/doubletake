...
def main():
    ...
    parser.add_argument(
        "--grit", action="store_true", help="download ScanNet files for General Robust Image Task"
    )
    parser.add_argument(
        "--type",
    ...
    if args.task_data:  # download task data
        ...
        download_tfrecords(
            os.path.join(BASE_URL, RELEASE_TASKS, "tf3d"), os.path.join(out_dir_tasks, "tf3d")
        )
